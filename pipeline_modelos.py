...
def avaliar_modelo_multiclasse(modelo, X_test, y_test, nome_modelo='Modelo'):
    ...
    plt.ylabel('Real')
    plt.tight_layout()
    plt.savefig(f'matrizes_confusao/matriz_confusao_{nome_modelo}.png')
    plt.close()
...
def NCV_treinar_modelos_robustos(df, nome_alvo, colunas_numericas, colunas_onehot,
    ...
    modelos = {
        ...
        'Rede_Neural': (
            MLPClassifier(max_iter=800, random_state=random_state),
            {
    ...
    joblib.dump(final_pipeline, f"modelos_salvos/modelo_final_{best_model_name}.pkl")
    print(f"Modelo final salvo como modelos_salvos/modelo_final_{best_model_name}.pkl")

    return df_resultados, final_pipeline, best_model_name



df = pd.read_csv("dataset_filmes_class.csv")

colunas_numericas = ['ano_lancamento', 'mes_seno', 'mes_cos', 'duracao', 'orcamento',
